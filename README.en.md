...
Jiaming is a local-first Chinese baby-naming application with traceable literary sources. It combines an interlinked wiki, a full-text corpus index, and a read-only LLM agent that verifies quotations before suggesting names.
...
- Read-only knowledge tools for wiki search, page reading, and original-text search.
- Two independent model providers:
  - Codex subscription through a logged-in Codex CLI; no API key required.
  - OpenAI-compatible Responses or Chat Completions endpoints; Codex CLI not required.
...
Copy `config.example.json` to `config.local.json` and `.env.example` to `.env`, then select either `codex` or `openai`. See the [Chinese configuration guide](docs/configuration.zh-CN.md) for all options.
