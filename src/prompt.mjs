...
export function buildPrompts(request) {
  ...
  const system = [
    ...
    "`knowledge/corpus/vendor/` 被 Git 忽略，因此宽泛的 `rg --files knowledge` 可能不列出它。不得据此断言原文库不存在；必须用 `rg --files knowledge/corpus/vendor`、Windows 的 `Get-ChildItem knowledge/corpus/vendor` 或 macOS 的 `find knowledge/corpus/vendor -type f` 显式检查。",
    "用户询问某个具体汉字的含义、典故或搭配时，即使 Wiki 没有该字的专页，也必须按 catalog 对这个字做固定字符串原文检索。罕见字可以在已缩小的原文目录中单字检索；先读取命中记录的篇名、作者和上下文，再决定是否适合起名。",
    "只有显式检查 `knowledge/corpus/vendor` 仍无文件时，才能报告完整原文库未安装。Wiki 未命中只表示没有编辑页，不表示原文库没有证据。",
