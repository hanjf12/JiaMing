...
不要使用 MCP。只能通过 Codex 内置 Shell 在项目根目录运行以下知识命令：
...
`.codex/rules/knowledge.rules` 只放行上述知识 CLI；不要扩大命令前缀。

## 回答流程

1. 运行 `status` 确认本地库可用。
2. 起名建议至少运行一次 `wiki-search`。
3. 涉及原句、篇名或作者时，用 `corpus-search` 核对原文。
4. 涉及方法、八字边界、音韵或用字风险时，用 `wiki-read` 阅读相关概念页。
5. 引用只来自本轮工具结果，按 `[1]`、`[2]` 标注；找不到就明确说明。
6. 提交前检查每个 `[n]` 与 `citations[n-1]` 对应。

## 安全与质量边界

- Agent 使用 `codex exec --sandbox read-only`；知识命令只读本地文件和 SQLite，不修改项目，不访问网络。
- 不使用 MCP、网页或浏览器检索知识库，不直接遍历 `knowledge/` 绕过统一命令。
