...
```text
find/list: rg --files --no-ignore knowledge
...
原文:      rg -n -F --no-ignore -m 8 -B 12 -A 5 "人间有味是清欢" <catalog 指定的语料目录>
```
...
3. 涉及原句、篇名或作者时，先读 `knowledge/corpus/catalog.md`，缩小到相关语料目录后检索原始 JSON。
   `knowledge/corpus/vendor/` 被 Git 忽略，文件发现和原文检索统一使用 `--no-ignore`；不要用 `-uu` 读入隐藏的 `.git` 元数据，也不能用普通命令判断原文库不存在。
